...
class RecordTests(APITestCase):
    ...
    def test_get_records(self):
        url = "".join([reverse("agent_recmii:record-list")])
        """
        正常系 1:
        正常にデータをgetできること
        """
        ...
        self.assertEqual(
            data["results"][0]["id"],
            str(self.record_2.id),
            "id カラムの検証.",
        )
        self.assertEqual(
            data["results"][0]["before_record_at"],
            self.record_2.before_record_at.astimezone(
                datetime.timezone(datetime.timedelta(hours=9))
            ).isoformat(),
            "before_record_at カラムの検証.",
        )
        self.assertEqual(
            data["results"][0]["is_long_time"],
            self.record_2.is_long_time,
            "is_long_time カラムの検証.",
        )
        self.assertEqual(
            data["results"][0]["is_checked"],
            self.record_2.is_checked,
            "is_checked カラムの検証.",
        )
        self.assertEqual(
            data["results"][0]["trigger"],
            self.record_2.trigger,
            "trigger カラムの検証.",
        )
        self.assertEqual(
            data["results"][0]["proposal_id"]["id"],
            str(self.record_2.proposal_id.id),
        ...
        with self.assertRaises(KeyError) as raises:
            data["results"][0]["proposal_id"]["jobseeker_id"]["in_charge_id"][
                "password"
            ]
    ...
    def test_search_jobseeker_id(self):
        ...
        url = "".join([reverse("agent_recmii:record-list")]) + f"?keyword={keyword}"
    ...
    def test_get_record(self):
        url = f"api/agent/v1/records/{self.record_1.id}/"
    ...
    def test_post_records(self):
        ...
        force_authenticate(request, user=self.user)
        view = RecordViewSet.as_view({"post": "create"})
    ...
    def test_put_records(self):
        ...
        force_authenticate(request, user=self.user)
        view = RecordViewSet.as_view({"put": "update"})
        response = view(request, pk=self.record_1.id)
    ...
    def test_delete_record(self):
        ...
        force_authenticate(request, user=self.user)
        view = RecordViewSet.as_view({"delete": "destroy"})
        response = view(request, pk=self.record_1.id)
